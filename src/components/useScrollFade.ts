 ...
 * scrolled up out of view (no fade while parked at the top). With
 * `{ fadeBottom: true }` the bottom edge fades the same way while there's more
 * content below; otherwise the bottom never fades.
 *
 ...
 */
export function useScrollFade({ fadeBottom = false }: { fadeBottom?: boolean } = {}) {
  ...
  const recompute = useCallback(() => {
    ...
    if (fadeBottom) {
      // Fade the bottom while content is still hidden below.
      const nextBottom = el.scrollTop + el.clientHeight < el.scrollHeight - 1;
      setShowBottomFade((prev) => (prev === nextBottom ? prev : nextBottom));
    }
  }, [fadeBottom]);
  ...
  const maskImage =
    ...
    `rgba(0,0,0,1) ${FADE}, rgba(0,0,0,1) calc(100% - ${FADE}), ` +
    `rgba(0,0,0,${showBottomFade ? 0 : 1}) 100%)`;
